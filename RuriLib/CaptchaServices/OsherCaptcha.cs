...
namespace RuriLib.CaptchaServices
{
    ...
    public class OsherCaptcha : CaptchaService
    {
        ...
        public OsherCaptcha(string apiKey, int timeout) : base(apiKey, timeout) { }
        ...
        public override double GetBalance()
        {
            using (CWebClient client = new CWebClient())
            {
                ...
                var response = client.DownloadString($"http://35.226.126.232/res.php?key={ApiKey}&action=getbalance");
        ...
        public override string SolveRecaptcha(string siteKey, string siteUrl)
        {
            ...
            {
                if (Timeout > 0) client.Timeout = Timeout;
                var response = client.DownloadString(string.Concat(new string[] { "http://35.226.126.232/in.php?key=", ApiKey, "&method=userrecaptcha&googlekey=", siteKey, "&pageurl=", siteUrl }));
                ...
                {
                    Thread.Sleep(5000);
                    response = client.DownloadString($"http://35.226.126.232/res.php?key={ApiKey}&action=get&id={TaskId}");
        ...
        public override string SolveCaptcha(Bitmap bitmap)
        {
            ...
            using (CWebClient client = new CWebClient())
            {
                ...
                var response = client.UploadString("http://35.226.126.232/in.php",
